...
那是**上限、不是承诺**：池子若超过 500 家（崩盘日的跌停池是有过的），不会少给，
而是**报错**（见表）。上游应答里的 **`data.tc` 才是真实家数**，
...
| 调用 | 行为 |
|---|---|
| 省略 `--limit` | 开 500 尽量取全；若仍被截断（`tc` > 返回条数）→ 抛 **`TRUNCATED`**，不静默少给 |
| 显式给 `--limit N` | 视为**有意**取前 N 条 → **只告警**（stderr）不改结果 |

`--limit` 因此**刻意没有默认值** —— 有默认值就分不清「有意截断」和「默认不够」，
而这两者一个只该提醒、一个该报错。

⚠️ 截断报错用的是 **`TRUNCATED`**，**不是 `NO_DATA`**。两者都会被调用方按 `code` 分支，
而语义正好相反：`NO_DATA` 是「当天没有」，`TRUNCATED` 是「有，但没给全」。
报成 `NO_DATA` 会让看板如实记下「9-18 没有涨停」—— 而实际是 78 家被扣住了。
